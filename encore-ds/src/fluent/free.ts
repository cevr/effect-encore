 ...
 * at most once.
 */
export function run<A>(action: RunAction<A>, options: RunOptions): Effect.Effect<A, never, FluentRequirements>;
export function run<A>(name: string, action: RunAction<A>): Effect.Effect<A, never, FluentRequirements>;
export function run<A>(
  ...
  const action = (typeof a === "string" ? (b as RunAction<A>) : a) as RunAction<A>;
...
export const race = <const Ops extends ReadonlyArray<Effect.Effect<any, any, any>>>(
  ops: Ops,
): Effect.Effect<Effect.Success<Ops[number]>, Effect.Error<Ops[number]>, Effect.Services<Ops[number]>> =>
