...
export const ActorStateRegistry = Context.GenericTag<
  ActorStateRegistryShape,
  ActorStateRegistryShape
>("effect-encore/actor-state/ActorStateRegistry");

export const ActorStateRegistryLive: Layer.Layer<ActorStateRegistryShape> = Layer.effect(
  ActorStateRegistry,
  Effect.gen(function* () {
...
export const watchStateOf = <State, Error = never, Requirements = never>(
  address: EntityAddress.EntityAddress,
): Stream.Stream<State, Error | ActorStateUnavailable, ActorStateRegistryShape | Requirements> =>
  Stream.unwrap(
    Effect.gen(function* () {
      const registry = yield* ActorStateRegistry;
      const handle = yield* registry.get(address);
...
export const listStateEntityIds = (
  entityType: string,
): Effect.Effect<ReadonlyArray<string>, never, ActorStateRegistryShape> =>
  Effect.gen(function* () {
    const registry = yield* ActorStateRegistry;
