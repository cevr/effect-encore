...
> =>
  Effect.context<Exclude<R, ActorLayerBuildContextExclusions>>().pipe(
    Effect.map(
      (ctx) =>
        Effect.provide(build, ctx) as Effect.Effect<
          A,
          E,
...
export type StateOf<S extends ActorStateDef | undefined> = S extends {
  readonly schema: infer Sch extends Schema.Schema.Any;
}
...
> = ActorOperationHandles<Name, Defs> &
  Pipeable.Pipeable & {
    ...
    readonly Control: Context.Tag<ActorControlClientService<Name>, ActorControlClient>;
    readonly State: Context.Tag<ActorStateClientService<Name>, ActorStateClient<State, StateError>>;
    /**
...
const makeWaitFor = <S, E>(
  ...
  // v3's `Effect.repeat({ schedule, while })` returns the schedule's Out, not
  // the effect's value. Use Stream-driven polling so we can both pace AND
  // return the last terminal result.
  const filter = options?.filter ?? (isTerminal as (r: PeekResult<S, E>) => boolean);
  const sched = options?.schedule ?? defaultWaitSchedule;
  return Stream.repeatEffectWithSchedule(peekFn(execId), sched).pipe(
    Stream.takeUntil(filter as (r: PeekResult<S, E>) => boolean),
    Stream.runLast,
    Effect.flatMap((opt) =>
      // Stream guarantees at least one element since repeatEffectWithSchedule
      // emits the first peek immediately. takeUntil keeps the matching item.
      opt._tag === "Some"
        ? Effect.succeed(opt.value)
        : Effect.dieMessage("waitFor: stream produced no values (unreachable)"),
    ),
  );
};
...
>(
  ...
  // eslint-disable-next-line typescript-eslint/no-unused-vars -- type-only carrier for State inference
  ...
  const stateTag = Context.GenericTag<
    ActorStateClientService<Name>,
    ActorStateClient<StateOf<StateDef>, StateErrorOf<StateDef>>
  >(`effect-encore/${name}/State`);

  const controlTag = Context.GenericTag<ActorControlClientService<Name>, ActorControlClient>(
  ...
  const decodeState = stateSchema
    ? (raw: unknown): Effect.Effect<unknown, unknown> =>
        Schema.decodeUnknown(stateSchema)(raw) as Effect.Effect<unknown, unknown>
    : (raw: unknown): Effect.Effect<unknown, unknown> => Effect.succeed(raw);
  const decodeError = errorSchema
    ? (raw: unknown): Effect.Effect<unknown, unknown> =>
        Schema.decodeUnknown(errorSchema)(raw) as Effect.Effect<unknown, unknown>
    : null;
    ...
    Stream.unwrap(
      Effect.gen(function* () {
        ...
        return watchStateOf(resolveEntityAddress(resolver, entityAny, entityId)).pipe(
          Stream.catchAll((cause: unknown) => Stream.fromEffect(decodeFailure(cause))),
...
>(
  ...
  | Snowflake.Generator,
  never,
  MessageStorage.MessageStorage | Sharding.Sharding | Scope.Scope | Rpc.MiddlewareClient<Rpcs>
>;
...
const makeActorControlLayer = <Name extends string, Defs extends OperationDefs>(
  ...
  ActorControlClientService<Name>,
  never,
  ActorAddressResolverShape | MessageStorage.MessageStorage
> =>
  Layer.effect(
    actor.Control,
    Effect.gen(function* () {
      ...
      const provideSupport = <A, E, R>(
        ...
        A,
        E,
        Exclude<R, ActorAddressResolverShape | MessageStorage.MessageStorage>
      > =>
        effect.pipe(
...
const makeActorStateLayer = <Name extends string, Defs extends OperationDefs, State, StateError>(
  ...
  ActorStateClientService<Name>,
  never,
  ActorAddressResolverShape | ActorStateRegistryShape | ActorClientService<Name, Defs>
> =>
  Layer.effect(
    actor.State,
    Effect.gen(function* () {
...
// Mirror of upstream `ClusterWorkflowEngine.entityAddressFor`. Both run reply
// and cached activity replies live at this address — `clearAddress` wipes all.
...
>(
  ...
  const peekById = (executionId: string): Effect.Effect<RawPeek, never, WorkflowEngine> =>
    Effect.map(
      wf.poll(executionId) as Effect.Effect<unknown, never, WorkflowEngine>,
      (pollResult): RawPeek => {
        if (pollResult == null) return Pending as RawPeek;
  ...
  const watchFn = (
    ...
    return Stream.unwrap(
      Effect.map(execIdFor(payload), (executionId) =>
        Stream.repeatEffectWithSchedule(peekById(executionId), Schedule.spaced(interval)).pipe(
  ...
  const executeFn = (payload: WorkflowPayloadType<Payload>) =>
    Effect.flatMap(execIdFor(payload), (executionId) =>
      Effect.flatMap(contextTag, (factory) =>
        Effect.flatMap(factory(executionId), (ref) => ref.execute(make(payload) as never)),
      ),
    ) as Effect.Effect<
      Schema.Schema.Type<Success>,
      Schema.Schema.Type<Error>,
  ...
  const sendFn = (payload: WorkflowPayloadType<Payload>) =>
    Effect.flatMap(execIdFor(payload), (executionId) =>
      Effect.flatMap(contextTag, (factory) =>
        Effect.flatMap(factory(executionId), (ref) => ref.send(make(payload) as never)),
      ),
    ) as Effect.Effect<
      ExecId<Schema.Schema.Type<Success>, Schema.Schema.Type<Error>>,
      never,
